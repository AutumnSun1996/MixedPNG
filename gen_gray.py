...
def gen_shadow_colored(
    ...
    :black_first: 优先保证黑色背景时的还原效果
        将导致另一背景下图像色彩较差
    """
    assert white_image.shape == black_image.shape
    # 处理允许分别指定/同时指定的参数
    ...
    if black_first:
        # 优先保证黑色背景下的显示效果
        result_bgr = np.nan_to_num(desaturated_b / alpha, 1) * 255.0
    else:
        # 优先保证白色背景下的显示效果
        result_bgr = np.nan_to_num((desaturated_w - 255.0) / alpha, 1) * 255.0 + 255.0
...
if __name__ == "__main__":
    path = {
        "white": R"D:\Pictures\Saved Pictures\Icon_LotusClean_Black.jpg",
        "black": R"D:\Pictures\Saved Pictures\SentientFactionIcon_b.png",
    }
    main(path)
